...
const DepartmentForm = require('../src/models/DepartmentForm');
const DepartmentAssignment = require('../src/models/DepartmentAssignment');
const DepartmentActivity = require('../src/models/DepartmentActivity');
...
async function getJobById(id) {
  ...
  const j = await Job.findOne({ id: numericId }).lean();
  if (!j) return null;
...
async function deleteJob(id) {
  const numericId = Number(id);
  await Job.deleteOne({ id: numericId });
  await DepartmentForm.deleteMany({ jobId: numericId });
}
    ...
    for (const [deptKey, customFields] of Object.entries(data.allDepartmentsData)) {
      ...
      for (const [k, v] of Object.entries(customFields)) {
        if (/engineer|person|fitter|wireman|painter|assembler|fabricator|done by|checked by|prepared by/i.test(k) && v) {
          if (!foundEngineers.includes(v)) {
            foundEngineers.push(v);
          }
        }
      }
...
async function updateJobField(id, department, field, value) {
  ...
  if (department === 'core') {
    if (['panelName', 'projectName', 'jobNo', 'qty', 'date', 'ttl', 'targetPosition'].includes(field)) {
      job[field] = value;
      await job.save();
    ...
    if (/engineer|person|fitter|wireman|painter|assembler|fabricator|done by|checked by|prepared by/i.test(field) && value) {
      ...
      if (assignment) {
        if (!assignment.assignedUsers.includes(value)) {
          assignment.assignedUsers.push(value);
...
async function updateJobDepartmentFields(id, department, fields, engineers = null, updater = null) {
  ...
  if (engineers !== null) {
    const assignment = await DepartmentAssignment.findOne({ jobId: Number(id), department });
    if (assignment) {
      assignment.assignedUsers = Array.isArray(engineers) ? engineers : [engineers];
      ...
      await DepartmentAssignment.create({
        jobId: Number(id),
        department,
        assignedUsers: Array.isArray(engineers) ? engineers : [engineers],
        assignedBy: updater,
        status: "Assigned"
      });
    }
...
async function allocateEngineers(id, department, engineers) {
  const assignment = await DepartmentAssignment.findOne({ jobId: Number(id), department });
  if (assignment) {
    assignment.assignedUsers = engineers;
    assignment.markModified('assignedUsers');
    await assignment.save();
  } else {
    await DepartmentAssignment.create({
