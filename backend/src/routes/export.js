...
const EXPORT_CONFIG = [
  ...
  },
  {
    ...
    columns: [
      { name: "Planned Date", get: j => j.fields?.design?.["Planned Date"] },
      { name: "Drawing Submission Date & Engineer Name", get: j => j.fields?.design?.["Drawing Submission Date & Engineer Name"] },
      { name: "Rev R0 GA Start Date", get: j => j.fields?.design?.["Rev R0 GA Start Date"] },
      { name: "Rev R0 GA End Date", get: j => j.fields?.design?.["Rev R0 GA End Date"] },
      { name: "Rev R0 GA Engineer", get: j => j.fields?.design?.["Rev R0 GA Engineer"] },
      { name: "Rev R0 SLD Start Date", get: j => j.fields?.design?.["Rev R0 SLD Start Date"] },
      { name: "Rev R0 SLD End Date", get: j => j.fields?.design?.["Rev R0 SLD End Date"] },
      { name: "Rev R0 SLD Engineer", get: j => j.fields?.design?.["Rev R0 SLD Engineer"] },
      { name: "Rev R0 BOQ Start Date", get: j => j.fields?.design?.["Rev R0 BOQ START DATE"] },
      { name: "Rev R0 BOQ End Date", get: j => j.fields?.design?.["Rev R0 BOQ END DATE"] },
      { name: "Rev R0 BOQ Engineer", get: j => j.fields?.design?.["Rev R0 BOQ Name of Person"] },
      { name: "Rev R0 Control Start Date", get: j => j.fields?.design?.["Rev R0 CONTROL START DATE"] },
      { name: "Rev R0 Control End Date", get: j => j.fields?.design?.["Rev R0 CONTROL END DATE"] },
      { name: "Rev R0 Control Engineer", get: j => j.fields?.design?.["Rev R0 CONTROL Name of Person"] },
      { name: "Rev R0 PDF", get: j => j.fields?.design?.["Rev R0 PDF"] },
      { name: "Client Submission Date R0", get: j => j.fields?.design?.["Client Submission Date R0"] },
      { name: "Rev R1 (Complete Group)", get: j => j.fields?.design?.["Rev R1 (Complete Group)"] },
      { name: "Rev R2 (Complete Group)", get: j => j.fields?.design?.["Rev R2 (Complete Group)"] },
      { name: "Final Approved Drawings Received Date", get: j => j.fields?.design?.["Final Approved Drawings Received Date"] || j.fields?.design?.["Final Approved Date"] },
      { name: "BOM Released To Purchase", get: j => j.fields?.design?.["BOM Released To Purchase"] || j.fields?.design?.["BOM Released"] || j.fields?.design?.["BOM Release Date"] },
      { name: "SO No. of ERP System", get: j => j.fields?.design?.["SO No. of ERP System"] || j.fields?.design?.["SO No. of ERP"] },
      { name: "BOM Prepare Engineer Name", get: j => j.fields?.design?.["BOM Prepare Engineer Name"] || j.fields?.design?.["BOM Prepare Name of Person"] }
    ]
  },
  {
    key: "mechanical",
    ...
    columns: [
      { name: "Fabrication Release Date", get: j => j.fields?.mechanical?.["Fabrication Release Date"] },
      { name: "Swati / Outsource Fabricator Name", get: j => j.fields?.mechanical?.["Swati / Outsource Fabricator Name"] },
      { name: "Fabrication Prepare Engineer Name", get: j => j.fields?.mechanical?.["Fabrication Prepare Engineer Name"] || j.fields?.mechanical?.["Fabrication Prepare Engineer"] },
      { name: "Door Details Sent Date", get: j => j.fields?.mechanical?.["Door Details Sent Date"] },
      { name: "Door Details Prepare Engineer Name", get: j => j.fields?.mechanical?.["Door Details Prepare Engineer Name"] || j.fields?.mechanical?.["Door Details Prepare Engineer"] },
      { name: "Execution File Date", get: j => j.fields?.mechanical?.["Execution File Date"] },
      { name: "Execution (PDF) Prepared Engineer", get: j => j.fields?.mechanical?.["Execution (PDF) Prepared Engineer"] || j.fields?.mechanical?.["Execution PDF Prepare Engineer"] },
      { name: "As Built Submission Date", get: j => j.fields?.mechanical?.["As Built Submission Date"] || j.fields?.production?.["As Built Submission Date"] },
      { name: "As Built Prepared by Engineer (Production)", get: j => j.fields?.mechanical?.["As Built Prepared by Engineer (Production)"] || j.fields?.production?.["As Built Prepared By Engineer (Production)"] },
      { name: "As Built (PDF) Prepared by Engineer (Design)", get: j => j.fields?.mechanical?.["As Built (PDF) Prepared by Engineer (Design)"] || j.fields?.production?.["As Built PDF Prepared By Engineer (Design)"] },
      { name: "Drawing Start Date", get: j => j.fields?.mechanical?.["Drawing Start Date"] },
      { name: "Drawing Complete Date & Release to Programme", get: j => j.fields?.mechanical?.["Drawing Complete Date & Release to Programme"] },
      { name: "Drawing Prepare Engineer Name", get: j => j.fields?.mechanical?.["Drawing Prepare Engineer Name"] },
      { name: "Door & Service Plate Programming Date", get: j => j.fields?.mechanical?.["Door & Service Plate Programming Date"] },
      { name: "Door & Service Plate Prepare Engineer Name", get: j => j.fields?.mechanical?.["Door & Service Plate Prepare Engineer Name"] },
      { name: "Drafting File Handover Date", get: j => j.fields?.mechanical?.["Drafting File Handover Date"] },
      { name: "DFT Prepare Engineer Name", get: j => j.fields?.mechanical?.["DFT Prepare Engineer Name"] },
      { name: "Release to Programme Date", get: j => j.fields?.mechanical?.["Release to Programme Date"] || j.fields?.mechanical?.["Programming Release to Punching & Laser"] },
      { name: "Programme Start Date", get: j => j.fields?.mechanical?.["Programme Start Date"] },
      { name: "Programme End Date", get: j => j.fields?.mechanical?.["Programme End Date"] },
      { name: "Programmer Engineer Name", get: j => j.fields?.mechanical?.["Programmer Engineer Name"] }
    ]
  },
  {
    key: "purchase",
...
router.get("/xlsx", async (req, res) => {
  try {
    ...
    let query = {};
    if (client) query.clientName = new RegExp(client, "i");
    if (search) query.jobNo = new RegExp(search, "i");

    const jobs = await db.getJobs(query);
    jobs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

    const wb = buildWorkbook(jobs, null, null, user);
    const buf = XLSX.write(wb, { type: "buffer", bookType: "xlsx" });

    res.setHeader("Content-Disposition", 'attachment; filename="job_planning_register.xlsx"');
